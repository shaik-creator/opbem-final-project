...
async function runMigrations() {
  try {
    await query("ALTER TABLE users MODIFY COLUMN role VARCHAR(100) NOT NULL DEFAULT 'Operations Staff'");
    await query("UPDATE users SET role = 'Admin' WHERE role IN ('Admin / Owner', 'Admin - full access') OR email = 'admin@orbem.local'");
    await query("UPDATE users SET role = 'Operations Staff' WHERE email IN ('ops@orbem.local', 'ops2@orbem.local')");
