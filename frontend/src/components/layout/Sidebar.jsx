...
import { PlaneTakeoff, Sparkles, UserRound } from 'lucide-react';
import { classNames } from '../../utils/formatters';
import { useAuth } from '../../services/authService';
import { getNavigationForRole } from '../../config/navigation';
import { ROLES, normalizeRole, roleLabel } from '../../constants/roles';
...
export default function Sidebar({ open, collapsed = true, onClose, onExpand, onCollapse }) {
  ...
  const settingsTarget = normalizedRole === ROLES.ADMIN ? '/settings' : '/profile';
  const settingsLabel = normalizedRole === ROLES.ADMIN ? 'Profile & Settings' : 'My Profile';
      ...
      >
        ...
        <div className="relative shrink-0 border-t border-white/10 p-3">
          ...
          >
            ...
            <span className={classNames('truncate transition-all duration-200', collapsed ? 'lg:w-0 lg:overflow-hidden lg:opacity-0' : 'opacity-100')}>{settingsLabel}</span>
            <Tooltip label={settingsLabel} collapsed={collapsed} />
          </NavLink>
        </div>
      </aside>
