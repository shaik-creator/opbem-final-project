        ...
        <table className="min-w-[920px] divide-y divide-[#dbe3ea] text-xs">
          ...
          <tbody className="divide-y divide-[#edf2f7]">
            {bookings.map((booking) => {
              ...
              const detailPath = selectionType === 'shipment' ? `/shipments/${booking.id}` : `/bookings/${booking.id}`;
              return (
              <tr key={booking.id} className={classNames('transition hover:bg-[#f8fafc]', isSelected ? 'bg-[#eff6ff]' : '')}>
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    <Link className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-[#dbe3ea] text-[#64748b] hover:bg-[#f5f7fb] hover:text-[#172033]" to={detailPath} aria-label="View booking">
