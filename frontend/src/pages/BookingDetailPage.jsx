...
export default function BookingDetailPage() {
  ...
  const canEditBooking = can(user?.role, 'booking.edit.assigned');
  const canUpdateShipment = can(user?.role, 'shipment.updateStatus') || can(user?.role, 'shipment.updateWarehouseStatus');
  const canViewRevenue = can(user?.role, 'revenue.view');
  const canEditRevenue = can(user?.role, 'revenue.edit');
  const canDeleteBooking = can(user?.role, 'booking.delete');
  const isShipmentRoute = location.pathname.startsWith('/shipments');
  const listPath = isShipmentRoute ? '/shipments' : '/bookings';
  const listLabel = isShipmentRoute ? 'Shipments' : 'Bookings';
  const editing = canEditBooking && searchParams.get('edit') === '1';
  ...
  return (
    <div className="space-y-4 text-[13px]">
      ...
      <div className="flex flex-wrap items-start gap-3">
        ...
        {canEditBooking && <Button icon={Edit} onClick={() => setSearchParams({ edit: '1' })} className="bg-[#0f1f3d] hover:bg-[#1a3258]">Edit booking</Button>}
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,0.85fr)]">
        ...
        <div className="space-y-4">
          ...
          {canViewRevenue ? <PaymentSummaryCard booking={data.booking} /> : null}
