    ...
    <div className="space-y-4 text-[13px]">
      ...
      {selectedIds.length ? (
        <div className="flex flex-wrap items-center gap-2 rounded-lg border border-[#bfdbfe] bg-[#eff6ff] px-3 py-2 shadow-sm">
          ...
          {isAdmin && <Button variant="secondary" icon={UserCheck} loading={bulkLoading === 'owner'} className="min-h-8 px-3 py-1.5 text-xs" onClick={() => bulkAssignOwner()}>Assign Owner</Button>}
          {isAdmin && <Button variant="secondary" icon={Download} className="min-h-8 px-3 py-1.5 text-xs" onClick={bulkExportSelected}>Export selected</Button>}
          {isAdmin && <Button variant="danger" icon={Trash2} loading={bulkLoading === 'delete'} className="min-h-8 px-3 py-1.5 text-xs" onClick={bulkDeleteSelected}>Delete selected</Button>}
        </div>
      ) : null}
      {loading ? (
