  ...
  return (
    <div className="space-y-5">
      ...
      <PageHeader
        ...
        actions={<><Button variant="secondary" icon={RefreshCw} onClick={loadTasks}>Refresh</Button>{isAdmin && <Button icon={Plus} onClick={() => setCreating(true)}>New task</Button>}</>}
      />
            ...
            return (
              <section key={column} className="rounded-lg border border-gray-200 bg-white p-3 shadow-card">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <h2 className="break-words text-sm font-semibold text-gray-900">{column}</h2>
                  ...
                  {rows.map((task) => (
                    <article key={task.id} className="rounded-md border border-gray-200 bg-gray-50 p-3 text-sm">
                      <div className="flex min-w-0 flex-wrap items-start justify-between gap-2">
                        <h3 className="break-words font-semibold text-gray-900">{task.title}</h3>
      ...
      <Modal title="New task" open={creating} onClose={() => setCreating(false)}>
        <form className="grid min-w-0 gap-4 md:grid-cols-2" onSubmit={createTask}>
          ...
          <label className="block text-sm md:col-span-2">
            ...
            <textarea className="min-h-24 w-full min-w-0 rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
          </label>
          <div className="flex flex-wrap justify-end gap-2 md:col-span-2"><Button type="submit" loading={saving}>Save task</Button></div>
        </form>
      </Modal>
    </div>
  );
